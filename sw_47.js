...
const BUILD_REVISION=2026090802;
...
const MUTABLE_CACHE_GRACE_MS=650;
...
/*
 * O primeiro frame pode usar uma cópia local já validada após uma curta janela
 * para a rede. A mesma requisição continua em segundo plano e renova o cache.
 * version.json fica fora: verificação de versão deve permanecer network-first.
 */
const FAST_STARTUP_PATHS=new Set([
  '/viewport_v10_10_9.js','/boot_v10.js','/config_v10_7.js','/update_v10_10_9.js','/app_v10_10_9_core.js',
  '/interaction_v10_10_9.js','/styles_v10_10_9.css','/modules/v107-core.js','/modules/v107-invites.js','/modules/v107-operations.js'
]);
...
async function refreshNavigation(request,event,fallback){
  try{
    ...
    if(response?.ok){await cacheHtml(response,fallback);return true;}
  }catch(error){}
  return false;
}
async function navigationFastStart(request,event){
...
async function networkFirstWithCacheGrace(request,event,{cacheName=SHELL_CACHE,timeout=MUTABLE_NETWORK_TIMEOUT_MS,grace=MUTABLE_CACHE_GRACE_MS}={}){
  const cache=await caches.open(cacheName),cached=await cache.match(request,{ignoreSearch:false});
  if(!cached)return networkFirst(request,{cacheName,timeout});
  const network=fetchWithTimeout(request,timeout,{cache:'no-cache'}).then(async response=>{if(response?.ok){await cache.put(request,response.clone());return response;}return null;}).catch(()=>null);
  const quick=await Promise.race([network,new Promise(resolve=>setTimeout(()=>resolve(null),grace))]);
  if(quick)return secureResponse(quick);
  event.waitUntil(network.then(()=>true).catch(()=>false));
  return secureResponse(cached.clone());
}
...
self.addEventListener('fetch',event=>{
  ...
  if(request.mode==='navigate'){event.respondWith(navigationFastStart(request,event));return;}
  ...
  /* Primeiro frame: rede ganha se responder rápido; cache bom evita espera longa. */
  if(FAST_STARTUP_PATHS.has(relativePath)){event.respondWith(networkFirstWithCacheGrace(request,event,{cacheName:SHELL_CACHE}));return;}
