/* Team Bulls v10.10.52 — Central canônica com propriedade histórica do relatório. */
'use strict';
(()=>{
  if(window.__TEAM_BULLS_TRAINER_CANONICAL_INBOX_101052__)return;
  window.__TEAM_BULLS_TRAINER_CANONICAL_INBOX_101052__=true;

  const VERSION='10.10.52-canonicalinbox3';
  const MAX_ITEMS=500;
  const CONCURRENCY=6;
  ...
  function mergeRows(canonicalRows,indexRows){
    const map=new Map();(indexRows||[]).forEach(row=>map.set(String(row.id),{...row}));
  ...
  async function refresh(force=false){
    ...
    loading=(async()=>{
      legacyTrainerMismatchCount=0;legacyAnsweredCount=0;ownerRecoveredCount=0;
  ...
  function hookHub(){
    const hub=window.TeamBullsTrainerHub;if(!hub||hub===hookedHub||hub.__tbCanonicalInbox101052)return !!hub;
    const wrapped=Object.freeze({...hub,__tbCanonicalInbox101052:true,async openInbox(){const result=await hub.openInbox.apply(hub,arguments);hookFilters();await refresh(true);return result;},async refreshInbox(){const result=await hub.refreshInbox.apply(hub,arguments);await refresh(true);return result;},async markAllRead(){return markAllRead();}});
  ...
  window.TeamBullsCanonicalTrainerInbox=Object.freeze({version:VERSION,refresh,render,open,markAllRead,state:()=>({uid:activeUid,count:items.length,lastLoadAt,loading:!!loading,legacyTrainerMismatchCount,legacyAnsweredCount,ownerRecoveredCount,rosterCount:rosterMap.size})});
})();
