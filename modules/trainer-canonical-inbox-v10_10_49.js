/* Team Bulls v10.10.49 — Central canônica tolerante a metadados legados. */
'use strict';
(()=>{
  if(window.__TEAM_BULLS_TRAINER_CANONICAL_INBOX_101049__)return;
  window.__TEAM_BULLS_TRAINER_CANONICAL_INBOX_101049__=true;

  const VERSION='10.10.49-canonicalinbox2';
  const MAX_ITEMS=500;
  const CONCURRENCY=6;
  ...
  function runtimeRoster(){
    try{
      const source=typeof TRAINER_STUDENTS!=='undefined'&&Array.isArray(TRAINER_STUDENTS)?TRAINER_STUDENTS:[];
      return source.map(row=>{const uid=String(row?.uid||row?.id||row?.studentId||'');return uid?{...row,uid,role:String(row?.role||'student')}:null;}).filter(Boolean);
    }catch(error){return[];}
  }
  async function loadRoster(uid){
    ...
    runtimeRoster().forEach(row=>{if(!map.has(row.uid)&&row.role==='student'&&(!row.trainerId||String(row.trainerId)===uid))map.set(row.uid,row);});
    const roster=[...map.values()];rosterMap.clear();roster.forEach(row=>rosterMap.set(row.uid,row));return roster;
  }
  ...
  async function canonicalForStudent(student,uid){
    ...
    try{
      ...
      (snap.docs||[]).forEach(doc=>{
        const data=doc.data()||{};
        if(String(data.studentId||'')!==sid||!questionnaireComplete(data))return;
        const trainerMismatch=!!data.trainerId&&String(data.trainerId)!==uid;
        const legacyAnswered=data.answered!==true;
        if(trainerMismatch)legacyTrainerMismatchCount++;
        if(legacyAnswered)legacyAnsweredCount++;
        rows.push({id:eventId('questionnaire',doc.id),type:'questionnaire',sourceId:doc.id,studentId:sid,title:titleFor('questionnaire',data),submittedDate:submittedDate('questionnaire',data),createdAt:sourceCreatedAt('questionnaire',data),_createdMs:sourceMs('questionnaire',data),canonical:true,legacyTrainerMismatch:trainerMismatch,legacyAnswered});
      });
  ...
  function mergeRows(canonicalRows,indexRows){
    ...
    (canonicalRows||[]).forEach(row=>{const indexed=map.get(row.id);map.set(row.id,{...indexed,...row,read:indexed?.read===true,readAt:indexed?.readAt||null,_createdMs:row._createdMs||indexed?._createdMs||0,canonical:true});});
  ...
  async function markRead(row){
    if(!row||row.read===true)return true;row.read=true;render();const ok=await ensureIndex(row);if(!ok)return false;
    try{await timeout(eventCollection(trainerUid()).doc(row.id).set({read:true,readAt:nowStamp()},{merge:true}),7000,'marcar relatório lido');return true;}catch(error){row.read=false;render();return false;}
  }
  async function open(rowId){
    ...
    try{
      ...
      const doc=await timeout(db.collection('questionnaires').doc(row.sourceId).get(),9000,'abrir relatório respondido');if(!doc.exists||!questionnaireComplete(doc.data()||{}))throw new Error('Relatório não encontrado.');
      TS_QUEST_CACHE=[{...doc.data(),answered:true,id:doc.id}];return viewQuestionnaire(doc.id,true);
  ...
  async function repairMissing(){
    if(repairRunning||!trainer())return repairRunning||Promise.resolve(false);const missing=items.filter(row=>row.canonical===true&&row.read!==true);
    repairRunning=(async()=>{await mapWithLimit(missing,REPAIR_CONCURRENCY,row=>ensureIndex(row));return true;})().finally(()=>{repairRunning=null;setTimeout(render,80);setTimeout(render,400);});return repairRunning;
  }
  async function refresh(force=false){
    ...
    loading=(async()=>{
      legacyTrainerMismatchCount=0;legacyAnsweredCount=0;
      const roster=await loadRoster(uid);const groups=await mapWithLimit(roster,CONCURRENCY,student=>canonicalForStudent(student,uid));
      const canonicalRows=groups.flatMap(group=>Array.isArray(group)?group:[]);const indexRows=await loadIndex(uid);
  ...
  function hookHub(){
    const hub=window.TeamBullsTrainerHub;if(!hub||hub===hookedHub||hub.__tbCanonicalInbox101049)return !!hub;
    const wrapped=Object.freeze({...hub,__tbCanonicalInbox101049:true,
  ...
  window.TeamBullsCanonicalTrainerInbox=Object.freeze({version:VERSION,refresh,render,open,markAllRead,state:()=>({uid:activeUid,count:items.length,lastLoadAt,loading:!!loading,legacyTrainerMismatchCount,legacyAnsweredCount,rosterCount:rosterMap.size})});
})();
