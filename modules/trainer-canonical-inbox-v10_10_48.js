/* Team Bulls v10.10.48 — Central do treinador derivada das fontes canônicas. */
'use strict';
(()=>{
  if(window.__TEAM_BULLS_TRAINER_CANONICAL_INBOX_101048__)return;
  window.__TEAM_BULLS_TRAINER_CANONICAL_INBOX_101048__=true;

  const VERSION='10.10.48-canonicalinbox1';
  const MAX_ITEMS=500;
  const CONCURRENCY=6;
  ...
  function sourceMs(type,data){
    return type==='questionnaire'?(stampMs(data?.answeredAt)||stampMs(data?.createdAt)):(stampMs(data?.createdAt)||stampMs(data?.answeredAt));
  }
  ...
  async function mapWithLimit(list,limit,worker){
    ...
    await Promise.all(Array.from({length:Math.min(Math.max(1,limit),Math.max(1,list.length))},run));
    return out;
  }
  ...
  async function loadRoster(uid){
    ...
    const roster=(snap.docs||[]).map(doc=>({...doc.data(),uid:doc.id})).filter(row=>row.role==='student'&&String(row.trainerId||'')===uid);
    rosterMap.clear();roster.forEach(row=>rosterMap.set(row.uid,row));return roster;
  }

  async function canonicalForStudent(student,uid){
    const sid=String(student?.uid||'');if(!sid)return[];
    const rows=[];
    try{
      ...
      (snap.docs||[]).forEach(doc=>{
        const data=doc.data()||{};
        if(data.answered!==true||String(data.studentId||'')!==sid)return;
        if(data.trainerId&&String(data.trainerId)!==uid)return;
        rows.push({id:eventId('questionnaire',doc.id),type:'questionnaire',sourceId:doc.id,studentId:sid,title:titleFor('questionnaire',data),submittedDate:submittedDate('questionnaire',data),createdAt:sourceCreatedAt('questionnaire',data),_createdMs:sourceMs('questionnaire',data),canonical:true});
      });
    }catch(error){console.warn('[Team Bulls] Relatórios canônicos indisponíveis para',sid,error?.code||error?.message||error);}
    try{
      ...
      (snap.docs||[]).forEach(doc=>{
        ...
        rows.push({id:eventId('weekly_checkin',doc.id),type:'weekly_checkin',sourceId:doc.id,studentId:sid,title:titleFor('weekly_checkin',data),submittedDate:submittedDate('weekly_checkin',data),createdAt:sourceCreatedAt('weekly_checkin',data),_createdMs:sourceMs('weekly_checkin',data),canonical:true});
      });
  ...
  async function loadIndex(uid){
    try{
      const snap=await timeout(eventCollection(uid).orderBy('createdAt','desc').limit(MAX_ITEMS).get(),9000,'estado lido da Central');
      return(snap.docs||[]).map(doc=>({...doc.data(),id:doc.id,_createdMs:stampMs(doc.data()?.createdAt),canonical:false}));
  ...
  function mergeRows(canonicalRows,indexRows){
    const map=new Map();
    (indexRows||[]).forEach(row=>map.set(String(row.id),{...row}));
    (canonicalRows||[]).forEach(row=>{
      const indexed=map.get(row.id);
      map.set(row.id,{...indexed,...row,read:indexed?.read===true,readAt:indexed?.readAt||null,_createdMs:row._createdMs||indexed?._createdMs||0,canonical:true});
    });
  ...
  function reportSection(){
    const body=document.getElementById('tb-inbox-body');if(!body)return null;
    return[...body.querySelectorAll('.tb-inbox-section')].find(section=>String(section.querySelector('.tb-inbox-section-title span')?.textContent||'').includes('RELATÓRIOS E ATUALIZAÇÕES RECEBIDOS'))||null;
  }
  function updateBadges(){
    if(!trainer())return;const unread=items.filter(row=>row.read!==true).length;
    document.querySelectorAll('[data-tb-inbox-count]').forEach(element=>element.textContent=unread?String(unread):'');
  }
  function render(){
    if(!trainer())return false;updateBadges();
    const body=document.getElementById('tb-inbox-body');if(!body)return false;
    const filter=currentFilter();if(filter==='updates')return true;
    let visible=items;if(filter==='unread')visible=visible.filter(row=>row.read!==true);
    let section=reportSection();
    if(!section){section=document.createElement('section');section.className='tb-inbox-section';body.appendChild(section);}
    section.innerHTML=`<div class="tb-inbox-section-title"><span>RELATÓRIOS E ATUALIZAÇÕES RECEBIDOS</span><span>${visible.length}</span></div>${visible.length?visible.map(row=>`<button class="tb-activity-card ${row.read===true?'':'unread'}" onclick="TeamBullsCanonicalTrainerInbox.open(${js(row.id)})"><span class="tb-activity-icon">${row.type==='weekly_checkin'?'▤':'◉'}</span><span class="tb-activity-main"><strong>${h(studentName(row.studentId))}${row.read===true?'':'<i class="tb-unread-dot"></i>'}</strong><span>${h(row.title||'Relatório recebido')}</span></span><span class="tb-activity-meta"><b>${h(fmtDate(row.submittedDate||''))}</b><small>${row.type==='weekly_checkin'?'SEMANAL':'SOLICITADO'}</small></span></button>`).join(''):'<div class="tb-empty">Nenhum item neste filtro.</div>'}`;
    return true;
  }
  ...
  async function markRead(row){
    if(!row||row.read===true)return true;row.read=true;render();
    const ok=await ensureIndex(row);if(!ok)return false;
    try{await timeout(eventCollection(trainerUid()).doc(row.id).set({read:true,readAt:nowStamp()},{merge:true}),7000,'marcar relatório lido');return true;}
    catch(error){row.read=false;render();return false;}
  }
  async function open(rowId){
    if(!trainer())return;const row=items.find(item=>item.id===String(rowId));if(!row)return;
    markRead(row).catch(()=>{});
    try{
      if(row.type==='weekly_checkin'){
        const doc=await timeout(db.collection('weeklyCheckins').doc(row.sourceId).get(),9000,'abrir relatório semanal');if(!doc.exists)throw new Error('Relatório não encontrado.');
        WEEKLY_CHECKINS=[{...doc.data(),id:doc.id}];return viewWeeklyCheckin(doc.id);
      }
      const doc=await timeout(db.collection('questionnaires').doc(row.sourceId).get(),9000,'abrir relatório respondido');if(!doc.exists||doc.data()?.answered!==true)throw new Error('Relatório não encontrado.');
      TS_QUEST_CACHE=[{...doc.data(),id:doc.id}];return viewQuestionnaire(doc.id,true);
  ...
  async function repairMissing(){
    if(repairRunning||!trainer())return repairRunning||Promise.resolve(false);
    const missing=items.filter(row=>row.canonical===true&&row.read!==true);
    repairRunning=(async()=>{await mapWithLimit(missing,REPAIR_CONCURRENCY,row=>ensureIndex(row));return true;})().finally(()=>{repairRunning=null;setTimeout(render,80);setTimeout(render,400);});
    return repairRunning;
  }

  async function refresh(force=false){
    if(!trainer())return[];const uid=trainerUid();if(!uid)return[];
    if(activeUid&&activeUid!==uid){items=[];rosterMap.clear();lastLoadAt=0;loading=null;}
    activeUid=uid;
    if(loading)return loading;
    if(!force&&items.length&&Date.now()-lastLoadAt<REFRESH_TTL_MS){render();return items;}
    loading=(async()=>{
      ...
      const canonicalRows=groups.flatMap(group=>Array.isArray(group)?group:[]);
      const indexRows=await loadIndex(uid);
      items=mergeRows(canonicalRows,indexRows);lastLoadAt=Date.now();render();repairMissing().catch(()=>{});return items;
    })().catch(error=>{console.error('[Team Bulls] Central canônica não pôde ser atualizada.',error);render();return items;}).finally(()=>{loading=null;});
    return loading;
  }
  ...
  async function markAllRead(){
    ...
    try{
      for(let offset=0;offset<pending.length;offset+=300){const batch=db.batch();pending.slice(offset,offset+300).forEach(row=>batch.set(eventCollection(trainerUid()).doc(row.id),{read:true,readAt:nowStamp()},{merge:true}));await timeout(batch.commit(),9000,'marcar Central como lida');}
      pending.forEach(row=>row.read=true);render();if(typeof showToast==='function')showToast('✓ Central marcada como lida');
    }catch(error){if(typeof showToast==='function')showToast('Não foi possível marcar tudo como lido.',true);}
  }

  function hookFilters(){
    if(filterHooked)return;const screen=document.getElementById('screen-trainer-inbox');if(!screen)return;
    filterHooked=true;screen.addEventListener('click',event=>{if(event.target?.closest?.('[data-inbox-filter]'))requestAnimationFrame(()=>render());});
  }
  function hookHub(){
    const hub=window.TeamBullsTrainerHub;if(!hub||hub===hookedHub||hub.__tbCanonicalInbox101048)return !!hub;
    const wrapped=Object.freeze({...hub,__tbCanonicalInbox101048:true,
      async openInbox(){const result=await hub.openInbox.apply(hub,arguments);hookFilters();await refresh(true);return result;},
      async refreshInbox(){const result=await hub.refreshInbox.apply(hub,arguments);await refresh(true);return result;},
      async markAllRead(){return markAllRead();}
    });
  ...
  function start(){if(!install())return;setTimeout(()=>{if(document.getElementById('screen-trainer-inbox')?.classList.contains('active'))refresh(true).catch(()=>{});},600);}

  start();
  [200,900,2200].forEach(delay=>setTimeout(install,delay));
  ...
  window.TeamBullsCanonicalTrainerInbox=Object.freeze({version:VERSION,refresh,render,open,markAllRead,state:()=>({uid:activeUid,count:items.length,lastLoadAt,loading:!!loading})});
})();
