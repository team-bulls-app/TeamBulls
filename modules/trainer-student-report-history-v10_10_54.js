/* Team Bulls v10.10.54 — histórico individual canônico com navegação imediata. */
'use strict';
(()=>{
  if(window.__TEAM_BULLS_TRAINER_STUDENT_REPORT_HISTORY_101054__)return;
  window.__TEAM_BULLS_TRAINER_STUDENT_REPORT_HISTORY_101054__=true;

  const VERSION='10.10.54-studentreports2';
  ...
  function patchReportDates(){
    ...
    if(renderQuestList.__tbTrainerAnsweredDate101054)return true;
    ...
    wrapped.__tbTrainerAnsweredDate101054=true;
  ...
  function setLoadingState(){
    const list=document.getElementById('ts-quest-list');
    const empty=document.getElementById('ts-quest-empty');
    if(list)list.innerHTML='<div class="no-data-inline">Carregando relatórios do aluno...</div>';
    if(empty)empty.style.display='none';
    const weekly=document.getElementById('ts-weekly-checkin-list');
    if(weekly)weekly.innerHTML='<div class="no-data-inline">Carregando relatórios semanais...</div>';
  }
  ...
  async function refreshStudentReports({navigation=null,showScreenNow=false}={}){
    ...
    const nav=navigation||(typeof beginAsyncNavigation==='function'?beginAsyncNavigation():null);
    if(showScreenNow&&typeof showScreen==='function')showScreen('screen-ts-quest',nav);
    setLoadingState();
    let questionnaires=[];
    try{questionnaires=await loadOwnedQuestionnaires(studentUid);}
      ...
      if(String(VIEW_STUDENT?.uid||'')===studentUid&&typeof showToast==='function')showToast('Não foi possível atualizar os relatórios deste aluno agora.',true);
      return false;
    }
    if(typeof isNavigationCurrent==='function'&&nav&&!isNavigationCurrent(nav))return false;
  ...
  function patchOpen(){
    ...
    if(openTsQuestionnaires.__tbTrainerOwnedHistory101054)return true;
    ...
    wrapped.__tbTrainerOwnedHistory101054=true;
