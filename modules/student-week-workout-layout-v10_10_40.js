...
'use strict';
(()=>{
  if(window.__TEAM_BULLS_STUDENT_DAY_WEEK_WORKOUT_LAYOUT_101040__)return;
  window.__TEAM_BULLS_STUDENT_DAY_WEEK_WORKOUT_LAYOUT_101040__=true;
  ...
  const html=value=>String(value??'').replace(/[&<>"']/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot',"'":'&#39;'}[char]));
  ...
  function prescriptionSummary(exercise,week){
    try{if(typeof prescriptionCompactSummary==='function')return prescriptionCompactSummary(exercise,week);}catch(error){}
  ...
  function injectStyles(){
    ...
    const style=document.createElement('style');style.id='tb-student-week-workout-layout-style';
    style.textContent=`
      ...
      #screen-day .tb-week-row:last-of-type{border-bottom:0}#screen-day .tb-week-row:active{background:rgba(137,31,33,.16)}#screen-day .tb-week-row.is-done{background:linear-gradient(90deg,rgba(40,94,61,.08),transparent 54%)}
      ...
      #screen-day .tb-week-no-rx{color:#6c625b;font-weight:600;font-size:9px}
      #screen-day .tb-week-rx-meta{display:flex;flex-wrap:wrap;gap:3px;align-items:center}
      ...
      #screen-day .tb-day-week-compare[aria-expanded="true"]{color:#c6b6aa;background:#15110f}
      #screen-day #student-day-weekly-board[hidden]{display:none!important}
      #screen-day #student-day-weekly-board:not([hidden]){border-top:1px solid #352e28;padding-top:2px}
      @media(max-width:420px){
        #screen-day .tb-week-sheet-head{grid-template-columns:40px minmax(0,1fr) 40px;min-height:72px}
        #screen-day .tb-week-heading strong{font-size:24px}#screen-day .tb-week-columns,#screen-day .tb-week-row{grid-template-columns:minmax(0,1.08fr) minmax(112px,.92fr) 15px;padding-left:8px;padding-right:8px}
        #screen-day .tb-week-columns span:nth-child(2){padding-left:8px}#screen-day .tb-week-prescription{padding-left:8px}#screen-day .tb-week-exercise>strong{font-size:13px}#screen-day .tb-week-prescription>strong{font-size:11px}#screen-day .tb-week-rx-chip{font-size:6px}
      }
      @media(min-width:900px){
        body.student-desktop #screen-day .weekly-plan-shell{max-width:980px}
        body.student-desktop #screen-day .tb-week-row,body.student-desktop #screen-day .tb-week-columns{grid-template-columns:minmax(250px,1.2fr) minmax(260px,.8fr) 22px}
        body.student-desktop #screen-day .tb-week-row{min-height:64px}body.student-desktop #screen-day .tb-week-exercise>strong{font-size:15px}body.student-desktop #screen-day .tb-week-prescription>strong{font-size:13px}
      }
    `;
  ...
  function ensureDaySheet(){
    const screen=document.getElementById('screen-day'),legacy=document.getElementById('student-day-weekly-board');
    const shell=legacy?.closest('.weekly-plan-shell');if(!screen||!legacy||!shell||!screen.contains(shell))return null;
    ...
    if(!host){
      ...
      host.addEventListener('click',event=>{
        const nav=event.target.closest?.('[data-week-delta]');if(nav){changeWeek(Number(nav.dataset.weekDelta)||0);return;}
        const row=event.target.closest?.('[data-week-exercise]');if(row){openExercise(row.dataset.weekExercise,clampWeek(row.dataset.week));return;}
        const compare=event.target.closest?.('[data-day-week-compare]');if(compare)toggleComparison();
      });
      legacy.insertAdjacentElement('beforebegin',host);
    }
    if(legacy.dataset.tbDayWeekCompareDefault!=='1'){
      legacy.dataset.tbDayWeekCompareDefault='1';legacy.hidden=true;
    }
    return{host,legacy,shell};
  }

  function syncComparisonButton(host,legacy){
    const button=host?.querySelector?.('[data-day-week-compare]');if(!button||!legacy)return;
    const expanded=!legacy.hidden;button.setAttribute('aria-expanded',String(expanded));button.textContent=expanded?'▦ OCULTAR COMPARAÇÃO DAS 8 SEMANAS':'▦ COMPARAR AS 8 SEMANAS';
  }
  function toggleComparison(){
    const ui=ensureDaySheet();if(!ui)return false;ui.legacy.hidden=!ui.legacy.hidden;syncComparisonButton(ui.host,ui.legacy);
    if(!ui.legacy.hidden){const context=activeDayContext();if(context)refreshLegacy(context);}
    return true;
  }
  function refreshLegacy(context){
    if(!context)return;
    try{
      if(typeof scheduleWeeklyBoardRender==='function')scheduleWeeklyBoardRender(context.dayWorkout,'student-day-weekly-board',false);
      else if(typeof buildWeeklyBoard==='function')buildWeeklyBoard(context.dayWorkout,'student-day-weekly-board',false);
    }catch(error){}
  }

  function render(context=activeDayContext(),week=0){
    if(!studentContext()||!context?.workout||!context?.day)return false;
    injectStyles();const ui=ensureDaySheet();if(!ui)return false;
    ...
    const range=cycleRange(context.workout,selected),current=isCurrentCycleWeek(context.workout,selected);
    const body=context.items.length?context.items.map(exercise=>rowHtml(exercise,context.workout,selected)).join(''):'<div class="prescription-empty">Nenhum exercício cadastrado nesta pasta.</div>';
  ...
  function setWeek(context,week,{refreshComparison=true}={}){
    if(!context?.workout)return 1;
    const next=clampWeek(week);selectedWeeks.set(workoutKey(context.workout),next);try{LAST_SESSION_WEEK=next;}catch(error){}
    render(context,next);if(refreshComparison&&!document.getElementById('student-day-weekly-board')?.hidden)refreshLegacy(context);return next;
  }
  function changeWeek(delta){
    const context=activeDayContext();if(!context)return false;
    const current=selectedWeek(context.workout),next=clampWeek(current+(Number(delta)||0));if(next===current)return false;
    setWeek(context,next);return true;
  }
  function openExercise(exerciseId,week){
    const context=activeDayContext();if(!context||!exerciseId)return false;
    if(!context.items.some(item=>String(item?.id||'')===String(exerciseId)))return false;
    const selected=setWeek(context,week,{refreshComparison:false});
    try{if(typeof openStudentWeekExercise==='function'){openStudentWeekExercise(exerciseId,selected);return true;}}catch(error){}
    try{if(typeof window.openExercise==='function'){window.openExercise(exerciseId);return true;}}catch(error){}
    return false;
  }

  function installRenderPatch(){
    if(typeof renderDay!=='function')return false;
    if(renderDay.__tbStudentDayWeekWorkoutLayout)return true;
    const base=renderDay;
    const wrapped=function(){
      const result=base.apply(this,arguments);
      if(studentContext())requestAnimationFrame(()=>{if(document.getElementById('screen-day'))render();});
      return result;
    };
    wrapped.__tbStudentDayWeekWorkoutLayout=true;wrapped.__tbBase=base;renderDay=wrapped;return true;
  }
  function install(){
    injectStyles();const ok=installRenderPatch();
    if(studentContext()&&document.getElementById('screen-day')?.classList.contains('active'))render();
    return ok;
  }
