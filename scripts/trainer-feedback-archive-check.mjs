...
has(mod,"db.collection('users').where('trainerId','==',uid).where('role','==','student').limit(500)",'Histórico deve restringir a query de usuários a alunos do treinador para ser compatível com Rules 28.');
...
assert(!/collection\('feedback'\)\.where\('studentId','==',student\.uid\)\.limit\s*\(/.test(mod),'Histórico completo não pode cortar feedbacks antigos por aluno com limit.');
...
console.log('APROVADO — histórico usa query de alunos compatível com Rules 28 e mantém todos os feedbacks próprios por aluno, com data, tipo, conteúdo e status; sem writes, polling ou corte do histórico.');
